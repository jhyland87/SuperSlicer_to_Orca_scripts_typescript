...
exports.calculatePrintParams = calculatePrintParams;
exports.detectIniType = detectIniType;
 ...
 * ```
 */
async function convertParams(parameter, file, sourceIni, status, newHash) {
    ...
    if (constants_1.multivalueParams[parameter]) {
        const array = (0, utils_1.multivalueToArray)(newValue);
        if (constants_1.multivalueParams[parameter] === 'single') {
            newValue = array[0] || '';
        }
        else {
            return array;
    ...
    const unbackslashGcode = () => {
        if (!newValue)
            return '';
        let val = newValue.replace(/^"(.*)"$/, '$1');
        return (0, utils_1.unbackslash)(val);
    };
    const handleCompatibleCondition = async () => {
    ...
    const specialCases = {
        ...
        filament_type: () => constants_1.filamentTypes[newValue] || newValue,
        filament_max_volumetric_speed: () => {
            const val = parseFloat(newValue);
            return val > 0 ? String(val) : (constants_1.defaultMVS[sourceIni['filament_type'] || ''] || '15');
        },
        draft_shield: () => {
        ...
        default_filament_profile: () => {
            const arr = (0, utils_1.multivalueToArray)(newValue);
            return (0, utils_1.unbackslash)(arr[0] || '');
        },
        retract_lift_top: () => {
            const arr = (0, utils_1.multivalueToArray)(newValue);
            return constants_1.zhopEnforcement[arr[0] || ''] || arr[0] || '';
        },
        compatible_printers_condition: () => handleCompatibleCondition(),
        compatible_prints_condition: () => handleCompatibleCondition(),
        max_layer_height: () => (0, utils_1.percentToMm)(String(status.value.nozzleSize || ''), newValue) || '',
        min_layer_height: () => (0, utils_1.percentToMm)(String(status.value.nozzleSize || ''), newValue) || '',
        fuzzy_skin_point_dist: () => (0, utils_1.percentToMm)(String(status.value.nozzleSize || ''), newValue) || '',
        fuzzy_skin_thickness: () => (0, utils_1.percentToMm)(String(status.value.nozzleSize || ''), newValue) || '',
        small_perimeter_min_length: () => (0, utils_1.percentToMm)(String(status.value.nozzleSize || ''), newValue) || '',
        bridge_flow_ratio: () => (0, utils_1.percentToFloat)(newValue),
        fill_top_flow_ratio: () => (0, utils_1.percentToFloat)(newValue),
        first_layer_flow_ratio: () => (0, utils_1.percentToFloat)(newValue),
        wall_transition_length: () => (0, utils_1.mmToPercent)(String(status.value.nozzleSize || ''), newValue) || '',
        ...
        support_material_style: () => {
            const style = constants_1.supportStyles[newValue];
            if (style) {
        ...
        fill_pattern: () => constants_1.infillTypes[newValue] || newValue,
        top_fill_pattern: () => constants_1.infillTypes[newValue] || newValue,
        bottom_fill_pattern: () => constants_1.infillTypes[newValue] || newValue,
        solid_fill_pattern: () => constants_1.infillTypes[newValue] || newValue,
        gcode_flavor: () => constants_1.gcodeFlavors[newValue] || '',
        host_type: () => constants_1.hostTypes[newValue] || newValue,
        thumbnails_format: () => constants_1.thumbnailFormat[newValue] || newValue,
        support_material_pattern: () => constants_1.supportPatterns[newValue] ? newValue : 'default',
        support_material_interface_pattern: () => constants_1.interfacePatterns[newValue] ? newValue : 'auto',
        seam_position: () => constants_1.seamPositions[newValue] || newValue,
        support_material_layer_height: () => parseFloat(newValue) > 0 ? '1' : '0',
        output_filename_format: () => newValue.replace(/\[/g, '{').replace(/\]/g, '}'),
        support_material_xy_spacing: () => {
            let calculated = (0, utils_1.percentToMm)(sourceIni['external_perimeter_extrusion_width'], newValue);
            if (!calculated) {
                calculated = (0, utils_1.percentToMm)(String(status.value.nozzleSize || ''), newValue);
            }
            return calculated || '';
        },
        ...
        external_perimeter_speed: () => (0, utils_1.percentToMm)(sourceIni['perimeter_speed'], newValue) || newValue,
        first_layer_speed: () => (0, utils_1.percentToMm)(sourceIni['perimeter_speed'], newValue) || newValue,
        top_solid_infill_speed: () => {
            const base = newHash['internal_solid_infill_speed'];
            return (0, utils_1.percentToMm)(typeof base === 'string' ? base : '', newValue) || newValue;
        },
        support_material_interface_speed: () => (0, utils_1.percentToMm)(sourceIni['support_material_speed'], newValue) || newValue,
        first_layer_infill_speed: () => {
            const base = status.slicerFlavor === 'PrusaSlicer'
                ? sourceIni['first_layer_speed']
                : newValue;
            return (0, utils_1.percentToMm)(sourceIni['infill_speed'], base) || newValue;
        },
        solid_infill_speed: () => {
            const base = status.slicerFlavor === 'PrusaSlicer'
                ? sourceIni['infill_speed']
                : defaultSpeed;
            return (0, utils_1.percentToMm)(base, newValue) || newValue;
        },
        perimeter_speed: () => {
            return status.slicerFlavor === 'SuperSlicer'
                ? ((0, utils_1.percentToMm)(defaultSpeed, newValue) || newValue)
                : newValue;
        },
        ...
        infill_speed: () => {
            const base = newHash['internal_solid_infill_speed'];
            return status.slicerFlavor === 'SuperSlicer'
                ? ((0, utils_1.percentToMm)(typeof base === 'string' ? base : '', newValue) || newValue)
                : newValue;
        },
 ...
 * ```
 */
async function calculatePrintParams(sourceIni, status, newHash) {
    // Translate and convert speed settings
    for (const parameter of constants_1.speedSequence) {
        if (!sourceIni[parameter])
            continue;
        ...
        if ((0, utils_1.isDecimal)(newValue)) {
            let formatted = parseFloat(newValue).toFixed(1);
            formatted = formatted.replace(/\.?0+$/, '');
            newHash[constants_1.speedParams[parameter]] = formatted;
        }
        else {
            newHash[constants_1.speedParams[parameter]] = newValue;
        }
    }
    ...
    newHash['wall_infill_order'] = (0, utils_1.evaluatePrintOrder)(status.toVar.externalPerimetersFirst, status.toVar.infillFirst);
    // Set ironing type
    newHash['ironing_type'] = (0, utils_1.evaluateIroningType)(status.toVar.ironing, status.ironingType);
    return newHash;
}
/**
 ...
 * ```
 */
function detectIniType(sourceIni) {
    if (sourceIni['ini_type']) {
        return sourceIni['ini_type'];
    }
    const typeCounts = {};
    for (const type of Object.keys(parameterMap_1.parameterMap)) {
        typeCounts[type] = 0;
        const typeMap = parameterMap_1.parameterMap[type];
        if (typeMap) {
